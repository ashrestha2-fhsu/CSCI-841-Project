...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // /users/profile can return { userId: number, ... } (backend),
        // while your User type uses { id: number, ... } in lists.
        const me = await userApi.profile() as any
        const currentUserId: number = me?.userId ?? me?.id

        const [usersData, accountsData, rolesData] = await Promise.all([
          userApi.getAll(),
          accountApi.getByUserId(currentUserId),
          roleApi.getAll()
        ])
  ...
  const activeAccounts = accounts.filter(a => (a as any).isActive).length
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-xl text-gray-600">Overview of your user management system</p>
        </div>
            ...
            {users.slice(0, 5).length > 0 ? (
              <div className="space-y-3">
                {users.slice(0, 5).map((user) => (
                  <div key={(user as any).id} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                    <div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
            ...
            <div className="space-y-3">
              {(['CHECKING', 'SAVINGS', 'CREDIT', 'INVESTMENT'] as const).map((type) => {
                const count = accounts.filter(a => (a as any).accountType === type).length
                return (
