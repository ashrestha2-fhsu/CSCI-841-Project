...
const AllAccountsModal: React.FC<AllAccountsModalProps> = ({
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      try {
        const res = await axiosInstance.get<Account[]>(`/accounts/user/${userId}/all`);
        setAllAccounts(res.data);
      } catch (err: any) {
        console.error("❌ Error fetching all accounts:", err?.response?.data || err?.message);
      }
    };
    fetchAll();
  }, [userId, isOpen]);
  ...
  return (
    <div className="modal-overlay">
      <div className="modals">
        <button onClick={onClose} className="closes-btn">X</button>
        <h2 className="header">All Accounts</h2>
        <table className="accounts-table">
          <thead>
            <tr>
              <th>Account Name</th><th>Type</th><th>Balance</th><th>Institution</th>
              <th>Account No.</th><th>Interest</th><th>Created</th><th>Actions</th>
            </tr>
          </thead>
          <tbody>
            {allAccounts.map((acc) => (
              <tr key={acc.accountId} className={acc.deleted ? "deleted-row" : ""}>
                <td>{acc.name}</td>
                <td>{acc.type}</td>
                <td>{acc.currency} {acc.balance}</td>
                ...
                <td>
                  {!acc.deleted ? (
                    ...
                    <button onClick={() => handleRestore(acc.accountId)}>Restore</button>
                  )}
                </td>
              </tr>
            ))}
